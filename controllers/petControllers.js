...
exports.createPet = async (req, res) => {
    const { name, species, breed, age, description, photo_url, pet_status } = req.body;
...
exports.getAllPets = async (req, res) => {
    const { page = 1, limit = 10, sort_by = 'pet_status', order = 'asc', name, species, breed, age, pet_status } = req.query;
...
exports.editPetById = async (req, res) => {
    const { id } = req.params;
    const { name, age, species, breed, description, petStatus, photo_url } = req.body;
    ...
    try {
        ...
        if (pet.user_id !== userIdFromToken) {
            return res.status(403).json({ error: 'You are not authorized to edit this pet' });
        }
